...
###4、其他
最近对于套打有了更新的进展，考虑到用户不是每个人都有扫描仪，我将算法做了更人性化的设计，可实现只要用手机拍摄物流单上传直接可以使用，对于缩放比率做了进一步更新。还有注意的是，每台打印机的偏移都不太一样，随着打印机的使用都会出现一定的偏移量，从程序上增加了总偏移量来解决偏移问题。目前这样的程序几乎已经能解决整个打印模块中会出现的问题。
